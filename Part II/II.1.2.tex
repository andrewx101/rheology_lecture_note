...
如果集合$X$的非空子集的集合$\mathcal{C}$满足$\bigcup_{Y\in\mathcal{C}}Y=X$且$\mathcal{C}$的元素两两不相交，则称集合$\mathcal{C}$是$X$的一个\emph{划分（partition）}。换言之，如果$X$的若干个非空子集两两不相交，但它们的并集又恰好得到$X$，那么这些子集就好像对集合$X$进行“切蛋糕”所得到结果（如图\ref{fig:II.1.2}右下的情况）。不难留意到$\mathcal{C}\subset\mathcal{P}\left(X\right)$。
...
若集合$\mathcal{C}$是集合$X$的一个划分，我们可以由此定义一个关系$\sim\equiv X/\mathcal{C}$\footnote{该记法与刚刚介绍完的$X/\sim$无关，是符号“$/$”的滥用。}，使得当且仅当$X$的元素$x,y$属于$\mathcal{C}$的同一个元素时，$x\sim y$\footnote{理解此句时注意$X$的划分是$X$的子集的集合，其元素是$X$的子集。}。图\ref{fig:II.1.2}中，将右下所示的集合的划分中同属一个子集的元素两两连线，就能得到左上所示的关系。容易验证，这种关系是等价关系。我们称该等价关系$\sim$是\emph{由集合$X$的划分$\mathcal{C}$引出的等价关系（equivalent relation induced by the partition $\mathcal{C}$ of $X$）}。\footnote{正式地，
    \[
...
可以证明，\emph{集合$X$上的任一划分都唯一地引出一个$X$上的等价关系}\footnote{\href{https://proofwiki.org/wiki/Relation_Induced_by_Partition_is_Equivalence}{证明过程}}。这里的“唯一”意思是说，从集合的同一个划分，不会引出多于一个不同的等价关系\footnote{注意，划分和等价关系都是集合。故它们“相同”即集合的相等；“不同”既集合的不相等。而集合的相等概念已经在上一节定义过了。}。

上一段介绍的是图\ref{fig:II.1.2}右下到左上的定理，下一定理则是图\ref{fig:II.1.2}从左上到右下的定理。
