...
\begin{definition}[对向量的导数、方向导数]\label{def:II.4.18}
...
\begin{theorem}\label{thm:II.4.10}
    ...
    \[\frac{\partial\mathbf{f}\left(\mathbf{x}\right)}{\partial\mathbf{y}}=\mathbf{L}_{\mathbf{x}}\mathbf{y},\quad\forall\mathbf{y}\in\mathbb{R}^n\]
    其中$\mathbf{L}_\mathbf{x}=\mathrm{d}_\mathbf{x}\mathbf{f}$是函数$\mathbf{f}$在$\mathbf{x}$处的导数。
\end{theorem}
\begin{proof}
    ...
    \begin{align*}
        ...
        \Leftrightarrow\lim_{t\to 0}\frac{\mathbf{f}\left(\mathbf{x}+t\mathbf{y}\right)-\mathbf{f}\left(\mathbf{x}\right)}{t}                                                                                                           & =\left(\mathrm{d}_\mathbf{x}\mathbf{f}\right)\mathbf{y}
    \end{align*}
\end{proof}
